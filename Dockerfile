# Base Image: La que especificaste, con Pytorch y CUDA [cite: 1]
FROM runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04

# --- PASO 1: INSTALAR DEPENDENCIAS DE SISTEMA (COMBINADAS) ---
# Se instalan las dependencias para ambos toolkits y las herramientas de gestión (Nginx, Supervisor)
RUN apt-get update && apt-get install -y --no-install-recommends \
    # Dependencias de gestión
    curl supervisor nginx \
    # Dependencias de text-animate-kit [cite: 15]
    chromium fonts-ipafont-gothic fonts-wqy-zenhei fonts-thai-tlwg fonts-kacst fonts-freefont-ttf \
    # Dependencias de nca-toolkit (para la compilación manual) [cite: 1, 2]
...
# Compilar e instalar FFmpeg con todas las librerías [cite: 8, 9, 10, 11]
...
RUN npm install
RUN npx playwright install chromium [cite: 15]
...
RUN pip install --no-cache-dir -r requirements.txt [cite: 13]
